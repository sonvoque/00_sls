...
/* Set 1 to enable power trace: only for sky */
...
#define PRINT_SENSOR		1

/*---------------------------------------------------------------------------*/
/* This is the Server UDP port used to receive data */
/* Response will be echoed back to DST port */
#define UDP_SERVER_LISTEN_PORT   	SLS_NORMAL_PORT

/* This is the  UDP port used to send async data to BR */
...
SLS_USING_HW = 4 : for compiling to Z1 used in Cooja simulation */

#define SLS_USING_HW	4
...
// redefine LEDs, check schematic for LEDs port
#undef	GPIO_BASE
#undef	LEDS_ALL
#undef	LEDS_GREEN 
#undef  LEDS_RED 
...
//if using host MCU communicating with CC2538 NCP, set this to TRUE and handle UART Rx data
#define CC2538DK_UART_COMM 		0
#define CC2538DK_HAS_SENSOR  	0

#if (CC2538DK_HAS_SENSOR==1)
#define CC2538DK_HAS_SHIELD
#endif


#endif /* SLS_USING_CC2538DK */
...
// If using Launchpad board: CC2650/CC13xx
...
// If using Z1: for simulation
#ifdef SLS_USING_Z1 		/* zolertia */
#define RED			LEDS_RED
#define BLUE		LEDS_BLUE
#define GREEN		LEDS_GREEN
#endif
...
//	err_code[1]: 	code returned in REPLY, sender check this field to know the REQ status
//	arg[n]: 		data payload
//	crc[2]			CRC check
...
struct cmd_struct_t {
	...
	uint8_t		cmd;
	uint8_t		err_code;
	uint8_t 	arg[MAX_CMD_DATA_LEN];	//18 byte
	uint16_t	crc;
};
